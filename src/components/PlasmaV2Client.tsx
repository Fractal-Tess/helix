...
export default function PlasmaV2({
  ...
  const propsRef = useRef<Partial<PlasmaV2Props>>({});
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (fadeStartTime.current !== null) {
        ...
        opacity = Math.min(fe / (fadeDur || 2000), 1);
        opacity = 1 - Math.pow(1 - opacity, 3);
      }
      uniformOffset.current[0] = xOff || 0;
      uniformOffset.current[1] = yOff || 0;
      program.uniforms.iTime.value = t;
      program.uniforms.uRotation.value = ((rot || 0) * Math.PI) / 180;
      program.uniforms.focalLength.value = fLen || 0.8;
    ...
    const start = () => {
      ...
      startTimeRef.current = performance.now() - (program.uniforms.iTime.value as number) * 1000;
      if (dynamicDpr) {
